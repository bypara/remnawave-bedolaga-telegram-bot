...
async def process_referral_registration(db: AsyncSession, new_user_id: int, referrer_id: int, bot: Bot = None):
    try:
        ...
        if bot:
            is_english = str(new_user.language or '').lower().startswith('en')
            referral_notification = (
                '<tg-emoji emoji-id="5461117441612462242">🙂</tg-emoji> '
                + ('<b>Welcome!</b>' if is_english else '<b>Добро пожаловать!</b>')
                + '\n\n'
                + (
                    f'You followed the referral link of <b>{html.escape(referrer.full_name)}</b>!'
                    if is_english
                    else f'Вы перешли по реферальной ссылке пользователя <b>{html.escape(referrer.full_name)}</b>!'
                )
            )
            if settings.is_referral_levels_scheme():
                from app.services.referral_reward_service import describe_referee_bonus

                referee_promise = await describe_referee_bonus(
                    db, language=new_user.language, referrer=referrer
                )
                if referee_promise:
                    referral_notification += (
                        f'\n\n🎁 Your bonus: {referee_promise}!'
                        if is_english
    ...
    try:
        ...
        if not user.has_made_first_topup:
            if not qualifies_for_first_bonus:
                logger.info(
                    'Пополнение меньше минимума первого бонуса, начисляется только комиссия',
                    user_id=user_id,
                    referrer_id=referrer.id,
                    topup_amount_kopeks=topup_amount_kopeks,
