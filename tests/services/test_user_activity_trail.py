...
def test_middleware_passes_telegram_id_and_keeps_tasks():
    """Batch writer keeps its worker alive and maps only Telegram IDs from bot events."""
    source = (Path(__file__).resolve().parents[2] / 'app' / 'middlewares' / 'button_stats.py').read_text(
        encoding='utf-8'
    )
    assert 'telegram_id=user_id' in source
    assert 'self._worker_task = asyncio.create_task(self._run())' in source
    assert 'User.telegram_id.in_(telegram_ids)' in source


def test_stars_payment_message_is_logged(monkeypatch):
    from app.middlewares.button_stats import ButtonClickEvent, ButtonStatsMiddleware, button_click_batch_writer

    middleware = ButtonStatsMiddleware()
    calls: list[ButtonClickEvent] = []
    monkeypatch.setattr(button_click_batch_writer, 'enqueue', calls.append)
