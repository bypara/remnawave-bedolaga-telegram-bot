"""Свежая установка и обновлённая обязаны прийти к одной схеме paritypay_payments.

Свежая база создаётся ``Base.metadata.create_all`` по модели, обновлённая —
миграцией 0114. Расхождение между ними живёт долго и тихо: autogenerate вечно
...
from app.database.models import Base, ParityPayPayment
...
def _load_migration():
    spec = importlib.util.spec_from_file_location('m0114', VERSIONS / MIGRATION)
...
def _fresh_install(path: pathlib.Path):
    ...
    Base.metadata.create_all(engine, tables=[ParityPayPayment.__table__], checkfirst=True)
    return engine
...
def test_order_id_is_unique(both):
    ...
    for inspector, label in zip(both, ('свежая', 'обновлённая'), strict=True):
        ...
        assert ('paritypay_payment_id',) in unique_columns, f'{label}: paritypay_payment_id не уникален'
